#!/usr/bin/env python3
"""Run fixed turn resolver A/B with joint-PBS terminal continuation values."""
...
def _local_ranges_from_belief(
    ...
    if belief_row is None:
        n = max(len(solver_hands), 1)
        return (
            np.ones(len(solver_hands), dtype=np.float32) / n,
            np.ones(len(solver_hands), dtype=np.float32) / n,
        )
...
def _pre_street_prefix(action_str: str, street: int) -> str:
    return "/".join(action_str.split("/")[:street])
    ...
    *,
    ...
    for action in _action_path(tree, node_idx):
        ...
        if action == 0:
            ...
            actor_spent = int(
                start_stack
                - int(tree["stacks_h"][child] if actor == 0 else tree["stacks_v"][child])
            )
...
class JointPBSTurnLeafCallback:
    def __init__(
        self,
        *,
...
def _solve_case(
    ...
    *,
    ...
    if backend != "cpu":
        raise ValueError("joint PBS resolver A/B requires CPU backend for leaf callbacks")
    ...
    callback = JointPBSTurnLeafCallback(
        case=case,
        board4=board_idx,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run fixed turn resolver A/B using joint-PBS learned terminal values."
    )
    ...
    metrics = {
        "mode": "joint_pbs_resolver_leaf_ab",
        "passed": bool(leaf_evaluated and all(record.get("passed") for record in evaluated)),
        "promotion_blockers": [
            "joint_pbs_resolver_leaf_ab_is_diagnostic_not_slumbot_confidence",
        ],
