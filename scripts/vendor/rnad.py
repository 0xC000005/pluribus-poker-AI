...
class RNaDSolver(policy_lib.Policy):
  ...
  def actor_step(self, env_step: EnvStep):
    ...
    # Vectorized categorical sampling (same distribution as per-row np_rng.choice,
    # but no Python loop over the batch -- this was ~half the rollout time).
    cdf = np.cumsum(pi, axis=-1)
    cdf[:, -1] = 1.0  # guard against floating-point drift so a draw always lands
    r = self._np_rng.random((pi.shape[0], 1))
    action = (r < cdf).argmax(axis=-1)
  ...
  def _batch_of_states_as_env_step(self,
    ...
    # Build batched arrays directly (avoids 256 per-game chex dataclasses + tree_map/np.stack,
    # ~a third of the rollout time). Same per-state extraction + dtypes as _state_as_env_step.
    if not hasattr(self, "_obs_dim_cache"):
      use_obs0 = self.config.state_representation == StateRepresentation.OBSERVATION
      self._obs_dim_cache = (self._game.observation_tensor_size() if use_obs0
                             else self._game.information_state_tensor_size())
      self._n_act_cache = self._game.num_distinct_actions()
      self._n_pl_cache = self._game.num_players()
    b = len(states)
    obs = np.empty((b, self._obs_dim_cache), dtype=np.float64)
    legal = np.empty((b, self._n_act_cache), dtype=np.int8)
    player_id = np.empty(b, dtype=np.float64)
    valid = np.empty(b, dtype=np.float64)
    rewards = np.empty((b, self._n_pl_cache), dtype=np.float64)
    use_obs = self.config.state_representation == StateRepresentation.OBSERVATION
    for i, st in enumerate(states):
      rewards[i] = st.returns()
      v = not st.is_terminal()
      valid[i] = v
      s = st if v else self._ex_state
      obs[i] = s.observation_tensor() if use_obs else s.information_state_tensor()
      legal[i] = s.legal_actions_mask()
      player_id[i] = s.current_player()
    return EnvStep(obs=obs, legal=legal, player_id=player_id, valid=valid, rewards=rewards)
