#!/usr/bin/env python3
"""Run a public-belief probe for per-hand counterfactual value targets."""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Compare feature-only and range-belief probes on searched "
            "counterfactual value vectors."
        )
    )
    ...
    parser.add_argument(
        "--train-cfv-cache",
        help="Optional .npz cache for computed train CFV labels and belief inputs.",
    )
    parser.add_argument(
        "--holdout-cfv-cache",
        help="Optional .npz cache for computed holdout CFV labels and belief inputs.",
    )
    parser.add_argument("--output-json")
    args = parser.parse_args(argv)

    from poker_ai.research.belief_value_probe import (
        run_public_belief_cfv_probe,
        save_metrics,
    )

    metrics = run_public_belief_cfv_probe(
