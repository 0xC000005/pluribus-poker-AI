...
- Build turn/river hand-sweep calibration targets: `python scripts/build_policy_calibration_targets.py --checkpoint models/control.pt --output autoresearch-session/policy_calibration/hand_sweep.npz --hand-sweep --sampled-cases 16 --hands-per-case 128 --strategy-source regret`
...
- Policy-head calibration targets are behavior-cloning diagnostics for range-likelihood calibration. Treat improved target loss or range dispersion as mechanism evidence only; do not promote without head-to-head, resolver, and Slumbot confidence evidence.
