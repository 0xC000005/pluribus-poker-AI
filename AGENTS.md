...
- Deep CFR can collect legal-mask average-strategy targets during traversal with `--average-strategy-weight > 0`, but this path is experimental and off by default. A first 5-iteration A/B with `--average-strategy-weight 0.1` regressed policy-head H2H, so do not scale it without a new falsification result.
