...
- Native NFSP-style pilots use the repo's full-deck 9-action contract and can run on CUDA, but they are still MC plumbing diagnostics rather than promoted agents. NFSP best-response/average-policy mode is sampled once per player per hand, not once per decision. On the RTX 3070 Ti, hidden 64 was CPU-faster, while hidden 512 was CUDA-faster in the first 200-episode A/B; record both throughput and payoff proxy before scaling.
