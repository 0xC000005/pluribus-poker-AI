...
- Native NFSP-style pilots use the repo's full-deck 9-action contract and can run on CUDA, but they are still diagnostics rather than promoted agents. NFSP best-response/average-policy mode is sampled once per player per hand, and the Q learner uses same-player next-decision DQN targets rather than assigning the final payoff to every action. On the RTX 3070 Ti, hidden 512 is CUDA-faster than CPU; record both throughput and payoff proxy before scaling.
