...
- `poker_ai/research/` contains the autoresearch workflow primitives, local evaluation helpers, search-target builders, fixed-state resolver benchmarks, promotion blockers, and Slumbot smoke parsers.
...
- Build resolver search targets: `python scripts/build_search_targets.py --output autoresearch-session/search_targets/fixed_turn_river.npz --solver-iterations 25 --solver-backend auto`
...
- Queue search-consistency training: `python scripts/poker_autoresearch.py enqueue-train --n-iterations 50 --n-traversals 4000 --search-targets autoresearch-session/search_targets/fixed_turn_river.npz --search-target-weight 0.05 --prefix search_consistency --save-every 25 --auto-compare`
