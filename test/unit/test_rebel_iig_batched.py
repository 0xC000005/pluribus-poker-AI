...
from poker_ai.rebel.iig_batched import solve_all_keys
...
def test_cross_key_batched_matches_numpy(name, game, cut, pkf):
    ...
    for k in keys:
        ...
        for iid in ref:
            max_l1 = max(max_l1, float(np.abs(np.asarray(ref[iid]) - np.asarray(got[k][iid])).sum()))
    assert max_l1 < 1e-4, f"{name}: cross-key batched vs numpy max L1 {max_l1:.2e}"
