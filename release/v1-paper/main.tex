...
against the same kernel run per-tree under eager dispatch; a measured six-arm
launch-amortization ablation --- \{fused, sequential\} $\times$ \{eager,
\texttt{torch.compile}, CUDA Graphs\} --- shows the per-solve advantage survives tool
symmetry: amortized-fused over amortized-sequential retains $\text{speedup} \approx N$
in the launch-bound regime (12.06$\times$ at Goofspiel-4, $N{=}12$) and
$\sim$1.8$\times$ at saturation. All claims are equal-band-faster, never lower
exploitability.
\end{abstract}
...
dispatch; the measured launch-amortization ablation shows that under tool symmetry ---
CUDA Graphs / \texttt{torch.compile} applied to \emph{both} arms --- fusion retains
$\text{speedup} \approx N$ in the launch-bound regime (12.06$\times$ at G4, $N{=}12$)
and $\sim$1.8$\times$ at saturation, all per-solve numbers (\S4.6, \S6.5). The demonstrated
...
per-key compiles plus the shared one (the \S6.5 ablation artifact now records the
analogous setup walls --- topology compile 0.09--9.4 s per game,
...
audited our own kernel against capturability: every tensor shape in the iteration body
...
remedies; the full audit, and the confirmation that those remedies are exactly what the
...
arm remains linear in the population size), \texttt{torch.compile}'s kernel re-codegen
...
\S6.1--\S6.4 are measured against an eager-dispatch single-stream sequential arm, and
we say so wherever they are claimed; \S6.5 is the tool-symmetric restatement.
...
We present five results, in the order of the argument: the cost-per-solve microbenchmark
...
matched full-configuration CPU timings, still planned (single-run CPU cells; \S4.6,
...
\textbf{Step 1 --- the asymmetric comparison (the one a skeptic would run first).}
...
comparison a skeptic would produce first, and because the asymmetry is exactly what
motivates step 2.
...
iterations; ms, GPU-event medians; best amortized arm per side in bold). Provenance:
the sequential arms and the eager-vs-eager reference come from the per-key session; the
amortized fused arms were measured in a completion session with the fused-eager arm
re-measured as the cross-session drift anchor (ratios 0.969--0.998 --- the completion
...
(8.77/11.69/2.37$\times$) within $\sim$2\%. Source:
\texttt{cuda\_graphs\_ablation.json} (original arms + \texttt{amortized\_fused}).}
\label{tab:sixarm}
...
\item \textbf{What this does not restate.} These are per-solve numbers under the
equal-band discipline of \S5 (parity-gated, timing-only). The end-to-end training
results of \S6.2 and \S6.4 (11.79$\times$/9.99$\times$ at G4; 2.40$\times$/2.00$\times$
at G5) remain eager-vs-eager in-kind comparisons and are deliberately not restated
under tooling; the amortized microbenchmark implies both e2e arms would speed up under
the same tooling, and the amortized-fused vs amortized-sequential \emph{end-to-end
training} comparison is future work (\S8).
\end{itemize}
...
$\sim$1.8$\times$ at saturation --- so the eager-tooling-class qualifier on the
$\sim$10$\times$ end-to-end figure is resolved at the per-solve level, and the
remaining item on this axis is the amortized-fused vs amortized-sequential
\emph{end-to-end training} run. Our own CPU-gate artifacts imply that at G4 scale the
GPU-sequential arm is $\sim$4.3$\times$ slower per belief than the same kernel on the
host CPU --- making the cross-substrate best-vs-best G4 win $\sim$2.7$\times$ as
currently measurable (single-run CPU cells, no timing discipline; \S4.6, Appendix~C);
matched full-configuration CPU timings (G4 and G5) remain planned. Multi-stream
per-tree dispatch and fp32-with-compensated-accumulation are further untested
...
(2) The remaining baseline items: matched CPU timings, and the amortized-fused vs
amortized-sequential \emph{end-to-end training} comparison (the
CUDA-Graphs/\texttt{torch.compile} cost-per-solve ablation is complete, both
directions, \S6.5); the quiet-host G5 timing re-run is complete ($n{=}3$, \S6.4),
...
certified on CPU, the CUDA band test pending). The measured launch-amortization
...
amortized end-to-end arms, and multi-device replication --- are complete. What we
...
\S6.5 (produces \texttt{cuda\_graphs\_ablation.json}). Every JSON cited in this paper is copied into
...
the timed region as honest tooling overhead. The manual-capture arm replays the
...
seed 0. The \S6.5 launch-amortization ablation has no seed axis (median-of-7-repeat
timings, like the \S6.1 microbenchmark; parity gates at 200 iterations). No other seeds
were run; no configuration sweeps were performed (single
