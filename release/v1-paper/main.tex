...
\author[1,2]{Max Jingwei Zhang\thanks{Correspondence: \texttt{maxjingwei.zhang@mail.utoronto.ca}.}\thanks{Work performed in Ontario Teachers' Pension Plan, Quantitative Modeling and Programming.}\thanks{The views expressed in this paper are solely those of the author and do not necessarily reflect the views, policies, positions, investment opinions, or risk-management practices of Ontario Teachers' Pension Plan or its affiliates.}}
\affil[1]{Ontario Teachers' Pension Plan}
\affil[2]{University of Toronto, Faculty of Applied Science and Engineering}
\date{June 10, 2026}
...
\begin{center}
\small
\textbf{Keywords:} Imperfect-Information Games; Counterfactual Regret Minimization;
Iterated Resolving; Public-Belief Search; GPU Acceleration; Batched Solvers
\end{center}

\begin{abstract}
Depth-limited resolving with learned value functions repeatedly solves populations of
public-belief subgames. At a fixed public cut, those subgames share continuation
topology and differ mainly in entry beliefs and solver state. This paper exploits that
structure with \textbf{population fusion}: compile the shared topology once and execute
each finite-budget CFR+ iteration for all subgames as batched tensor operations. A
floor/slope timing model predicts when fusion amortizes launch and short-kernel overhead
and when it becomes slope-ratio-limited. In Goofspiel-4, a tool-symmetric
\texttt{torch.compile} comparison gives 11.36$\times$ inner and 7.38$\times$ total-wall
speedup over 5 matched CUDA seeds, matching a 7.39$\times$ Amdahl ceiling and staying
within the predeclared descriptive band check. In saturated Goofspiel-5, a quiet-host
paired run gives 2.40$\times$ inner and 2.00$\times$ total timing speedup. The claim is
population-level cost-per-solve amortization for iterated resolving, not lower
exploitability or poker-bot strength.
\end{abstract}

\section{Introduction}

Depth-limited resolving makes search usable in large imperfect-information games: solve
a small lookahead game, use a value function at the depth limit, and repeat. This is the
template behind DeepStack's continual re-solving with learned counterfactual
values~\citep{Moravcik2017}, ReBeL's public-belief search and reinforcement
learning~\citep{Brown2020}, and Student of Games' search-learning-game-theoretic
unification~\citep{Schmid2023}. In training, however, the template creates a hidden
systems cost. The agent repeatedly asks a solver to produce value targets for many
public-belief subgames: small continuation games rooted at the same public history but
conditioned on different beliefs over hidden information. Those beliefs matter because,
unlike in chess or Go, an imperfect-information position does not have a single value
unless it is tied to what both players might privately hold and believe. The public
history plus those belief distributions is the \emph{public belief state} (PBS), and
naively truncating search without belief-conditioned leaf values yields exploitable
strategies~\citep{BrownSandholm2018}.

The important systems fact is that one resolving sweep does not produce one arbitrary
subgame at a time. It produces a \emph{subgame population}. At a fixed public cut, each
public outcome, or key, roots a continuation subgame. In the loop studied here these
continuation trees share topology; what changes from key to key is the data flowing
through the tree: the two players' entry beliefs, or \emph{ranges}, and the solver's
per-subgame state. Running the same GPU solver separately for every key therefore
repeats the same short-kernel launch sequence many times. For small subgames this is
launch-bound: the GPU can be underfilled even though the training loop spends almost all
of its time resolving.

This paper studies that repeated target-generation step as a cost-per-solve problem. The
wall-clock cost of the resolving phase factors as
\begin{equation}
...
$T$ is the per-solve CFR+ iteration budget, and $c_{\text{solve}}$ is wall-clock per
solve-iteration. The loop's remaining phases --- network training and measurement ---
sit outside this factor and are handled by the Amdahl analysis of \S6.2. In our
Goofspiel-4 (G4) operating point, the inner re-solve accounts for 98.4\% of baseline
loop wall-clock (\S3).

\textbf{One factor of Eq.~(\ref{eq:cost}) is under-isolated.} Existing work makes
resolving practical by changing how many subgames are solved, how deep or large each
subgame is, how many iterations are needed, or how one tree's CFR computation is
parallelized (\S3, \S7). Recent GPU-CFR work, including NoRegret-style linear-algebra
CFR~\citep{Kim2024,KimSandholm2026} and real-time HUNL-oriented parallel
CFR~\citep{RealTimeParallelCFR}, is primarily a within-tree or single-solve systems
story. The missing systems unit here is the resolving sweep: amortize
$c_{\text{solve}}$ across the same-topology subgame population generated by one sweep,
not only inside one member of that population.

\textbf{The mechanism is across-key population fusion.} Many same-shaped small problems
differing only in data are the workload that batched BLAS~\citep{Abdelfattah2016,
Abdelfattah2021} and FlashAttention-style kernel engineering~\citep{Dao2022} teach us
to exploit: fuse them, so the device performs the same arithmetic under far fewer kernel
launches. We call the resulting mechanism \textbf{population fusion}: compile the shared
subgame topology once into a level-grouped structure-of-arrays (SoA), fold the public-key
population into the tensor dimensions, and execute each finite-budget CFR+ iteration for
all keys as one fixed sequence of batched tensor ops (Figure~\ref{fig:hero}; \S4). The
inner solver is CFR+~\citep{Tammelin2014,Tammelin2015,Bowling2015}, descended from
CFR~\citep{Zinkevich2007}. The ``GEMM'' in the title names the systems pattern --- many
same-shaped solves turned into one batched tensor workload --- not literal matrix
multiplication at every step. The fusion axis is $K$, the number of public keys at the
cut; the within-key private-hand/range dimension is classical range-vector
CFR~\citep{Johanson2011,Johanson2012} and is present in both timing arms. The fused
solver is verified to match the per-tree finite-iteration CFR+ outputs under the gates
of \S4.5.
...
evaporate inside end-to-end systems; this one survives because the loop is dominated by
the phase we fused. On Goofspiel-4 --- small subgames, the launch-bound regime --- the
fused inner re-solve is 11.79$\times$ faster than the same kernel applied per tree,
against the 11.7$\times$ the microbenchmark sweep predicted ex ante, and total training
wall-clock improves 9.99$\times$ against the 10.09$\times$ Amdahl ceiling implied by the
measured inner fraction (5 matched seeds; \S6.2). A tool-symmetric
\texttt{torch.compile} comparison gives 11.36$\times$ inner and 7.38$\times$ total-wall
speedup (\S6.5), so the result is not merely an eager-dispatch artifact; CUDA Graphs are
the rival launch-amortization lever considered in that analysis~\citep{CUDAGraphs}. The
speedup buys no lower-exploitability claim: the arms pass the paper's broad descriptive
band check --- fused median 0.0402, sequential 0.0469, where for scale uniform random
play scores $\approx$1.42 on this game and 0 is exact Nash. On Goofspiel-5, where one
...
This section reviews the three prerequisites the rest of the paper assumes: how CFR+
...
give $B = 64$ \emph{cut instances} at G4 (27 at G3, 125 at G5). Thus $K$ counts public
outcomes, while $B$ counts the compatible private-deal rows carried through the shared
topology.
...
sound or sound-motivated instantiations --- DeepStack's continual
re-solving~\citep{Moravcik2017},
...
$N_{\text{solves}}$ & total subgame solves over a training run \\
$L$ & number of level-shape groups in the compiled solver \\
...
Three project terms used throughout: a \textbf{gate} is a hard pass/fail check
...
below are measured in the experiments of \S6 (artifacts described in the
Reproducibility Statement and Appendix~F); this section states
...
\textbf{The lever taxonomy, compressed.} Prior work attacks or reduces every factor of
...
attacked mainly \emph{within} a single tree, by the GPU-CFR
line~\citep{Kim2024,KimSandholm2026}; distributed systems have also solved independent
subgames on separate workers, which is task-level parallelism rather than a
single-device fused CFR iteration. The cost-per-solve-across-the-population axis ---
...
The mechanism has three parts. (1) All $K$ cut subgames share one topology,
...
identical four update steps and update semantics, differing in exactly one line. (Shape
...
The two algorithms use the same kernel family and update semantics on the same per-key
data; they differ in exactly one visible line --- the host loop over keys versus the
population tensor dimension. This is
...
\emph{Invariant:} restricted to any one public key, the fused tensors are the solo CFR+
tensors. Fusion forms a direct sum over public keys: disjoint regret/strategy rows and
shared topology groups, with the key population carried as data. No update for one key
reads another key's rows.
...
floating-point accumulation order and op scheduling. In linear-algebra terms this is a
direct sum over public keys. The only non-additive scatter is the own-reach assignment
into an infoset row; perfect recall makes that assignment single-valued within an
infoset, with the full argument in Appendix~A. This is why parity is mathematically
expected:
...
comparison a weak baseline. The baseline hierarchy is: primary comparator = same-kernel
per-tree GPU arm; boundary comparators = host CPU and launch-amortized sequential arms;
rejected timing comparator = Python tree walk, used only for correctness. Any reported
fused-vs-sequential speedup is thus attributable to
...
do not report the midpoint as a robust statistic. Rounding conventions and the rationale
...
have no evidence of systematic bias at this seed count: the G4 seed signs (fused higher
on 2/5, lower on 3/5) are descriptive and underpowered --- \textbf{a sign test at this
$n$ cannot exclude moderate bias} --- and pooling the quiet-host G5 pairs
...
Leduc and liar's dice, is $0$--$7\times10^{-6}$). The measured single-solve outputs
match under the parity gate.
...
\S6.4.
\item \textbf{Descriptive band check.} This is not an improvement claim. Fused final
NashConv median \textbf{0.0402} [10th/90th: 0.0238, 0.0533]; sequential median \textbf{0.0469}
...
evidence of systematic bias at this seed count (fused higher on 2/5, lower on 3/5; $n{=}5$ is
...
Figures~\ref{fig:e2ewall} and~\ref{fig:band} show the phase split and final band check.
...
\caption{\textbf{Descriptive band check; underpowered sign tally ($n{=}5$).} Final exact NashConv
...
--- so reporting it as the search-free boundary would overstate the resolving loop's
relative position; we therefore do not use it. VRPO is cited as the strongest
search-free boundary we are
...
\subsection{Goofspiel-5: fused-only target band and separate paired timing point}
...
\begin{table}[t]
\centering
\caption{Goofspiel-5 per-belief finite-budget target band (full 8-round protocol,
fused path only; not a fused-vs-sequential comparison).}
\label{tab:g5band}
\begin{tabular}{cccc}
\toprule
...
points. Recent GPU-CFR work, including Real-Time Parallel CFR~\citep{RealTimeParallelCFR},
parallelizes the iteration within a single game tree or depth-limited solve through
linear algebra, CPU/GPU pipeline stages, pruning, abstraction, and GPU leaf evaluation.
The linear-algebra formulation naturally suggests additional batching dimensions; our
claim is not that batching is unavailable to such systems. Rather, we isolate the
population generated by train-time resolving, integrate it into a parity-checked CFR+
loop with shared range-vector state, and validate a regime model end to end. We are not
aware of prior published work that evaluates this population-level fusion in an
iterated-resolving training loop.
...
\textbf{Scope of the demonstrated loop.} The end-to-end demonstration is a
\textbf{2-level, single-cut} iterated-resolving loop: the trunk is re-solved against
value targets obtained by independently re-solving each cut subgame at sampled beliefs.
This is the setting where the same-topology population is explicit and fully fusable.
The contribution is the conjunction of this structural identification, a parity-gated
level-grouped CFR+ implementation, the floor/slope regime model, and an
Amdahl-surviving end-to-end timing result. We do not claim that batching itself is new,
nor that the result already covers serially coupled multi-level resolving. DeepStack-style
independent target generation is fully fusable; ReBeL-style episodic resolving is
serially coupled along an episode, though still fusable across parallel episodes, keys,
or beliefs. The strongest form of the claim --- fused-vs-sequential \textbf{timing
under $\geq$3-level nested resolving} --- is deferred to future work. A supplementary
Goofspiel-4 multi-level self-play measurement shows decreasing exploitability
(0.0506 $\to$ 0.0145), but it is not headline timing evidence.
...
not yet instrumented. On a data-center part with 1:2 FP64 (A100) and $\sim$10$\times$
the FP64 throughput, both the effective launch floor and the saturation point would
change; the model predicts the outcome only after measuring the regime probe on that
device. On this device, the both-ends validation and the HUNL-mass argument below hold
as measured; their numeric values are device-conditional, and A100 replication is
future work.
...
replication and multi-GPU sharding --- the population axis provides a natural sharding
dimension, though communication, scheduling, and memory costs remain to be measured.
(5) A HUNL-scale instance built on lazy subgame construction, where
...
Train-time iterated resolving structurally generates same-topology public-belief
subgame populations. \textbf{Population fusion} turns that population into one
level-grouped CFR+ tensor workload, reducing the cost per finite-budget solve while
preserving per-tree CFR+ outputs under the paper's parity gates. The floor/slope model
explains the two observed regimes: near-linear speedup when the per-tree GPU workload is
launch-bound, and a smaller slope-ratio win once one subgame already fills the device.
The end-to-end Goofspiel-4 and Goofspiel-5 measurements support that model under matched
seeds and explicit Amdahl accounting.

The claim is deliberately narrow. This is not a stronger game-playing policy, a lower
exploitability result, or a universal speedup claim. It is a measured cost-per-solve
reduction for same-topology PBS subgames in iterated resolving. The pieces intended to
generalize are the abstraction, the floor/slope regime model, and the evaluation
template: parity-gate the solver on CPU, compare matched same-kernel arms, account for
non-resolve phases, and treat compounding CUDA atomics as a distributional timing
evaluation problem rather than a per-seed equality test.

\section*{Reproducibility Statement}

\textbf{Code, commits, and artifacts.} All experiments live in one repository (release
accompanies the preprint). Appendix~F lists the evidence-bearing commits, one-command
reproduction lines, seeds, fixed configurations, aggregation rules, hardware/software
stack, and total-compute accounting. Every artifact cited in the paper is copied into
the tracked directory \textbf{\texttt{release/v1-artifacts/}} with a
\texttt{README.md} manifest and per-file SHA-256 sums; those files are the
reproducibility bundle that accompanies the paper.
...
\paragraph{Affiliation disclaimer.}
The views and opinions expressed in this paper are solely those of the author and do not
necessarily reflect the views, policies, positions, investment opinions, or
risk-management practices of Ontario Teachers' Pension Plan, its affiliates, or any
other institution with which the author is associated. This paper is for research
purposes only and does not constitute investment advice, risk advice, an offer, or a
solicitation to buy or sell any financial instrument.

\paragraph{Competing interests.}
The author declares no competing interests.

\paragraph{Generative AI usage declaration.}
Generative AI tools, including OpenAI ChatGPT and Codex, were used to assist with
manuscript drafting, editing, code navigation, formatting, and verification workflows.
The author reviewed, edited, and verified the manuscript, experiments, figures,
citations, and code outputs, and remains solely responsible for the paper's content,
scientific claims, experimental results, and interpretation.
...
report the per-seed values themselves and do not report the midpoint as a robust
...
We audited the solver for CUDA-Graph capturability before measuring. Per
...
\texttt{torch.compiler.\allowbreak cudagraph\_mark\_step\_begin()} before each iteration plus
...
Appendix~E provides additional context for adjacent work; the claim-bearing
delimitations are in \S7.
